...
import TemplateEditorModal from '@/Projects/Project/modules/logs/TemplateEditorModal';
import ConfirmDialog from '@/components/ui/ConfirmDialog';
import Button from '@/components/layout/Button';
import LogGroupContainer from '@/Projects/Project/modules/logs/LogGroupContainer';
...
export default function LogsPage() {
  ...
  const { templates, create: createTemplate, remove: removeTemplate, addField, updateField, removeField, reorderFields } = useTemplates(activeProjectId);
  const { groups, create: createGroup, rename: renameGroup, remove: removeGroup } = useLogGroups(activeProjectId);
  ...
  async function handleCreateTemplate() {
    if (!newTemplateName.trim() || !activeProjectId) return;
  ...
  async function handleConfirmDeleteTemplate() {
    ...
    if (!result.ok) setTemplateDeleteError(result.reason);
    setTemplateToDelete(null);
  }

  if (loadingProjects) return <p>Carregando...</p>;

  if (projects.length === 0) {
    return (
      <div style={{ maxWidth: 720, margin: '2rem auto' }}>
        <h1>Logs</h1>
        <p style={{ color: '#666' }}>Você precisa de pelo menos um projeto antes de criar logs. Crie um projeto primeiro.</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 720, margin: '2rem auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div>
        <label style={{ fontSize: 12, fontWeight: 600, color: '#666', display: 'block', marginBottom: 4 }}>Projeto</label>
        <select
