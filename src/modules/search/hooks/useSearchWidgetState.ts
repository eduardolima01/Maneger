...
export function restoreFocusAfterSearchClose() {
  previousActiveElement?.focus?.();
  previousActiveElement = null;
}

export function useSearchWidgetState() {
  ...
  const openSearch = useCallback(() => setGlobalOpen(true), []);
  const closeSearch = useCallback(() => {
    setGlobalOpen(false);
    restoreFocusAfterSearchClose();
  }, []);

  return { open, openSearch, closeSearch, openToken };
}
