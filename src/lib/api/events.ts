import { CreateEventInput, Event, UpdateEventInput } from '@/types/event.types';
...
export async function createEvent(input: CreateEventInput): Promise<number> {
  ...
  const result = await db.execute(
    'INSERT INTO events (project_id, title, start_at, end_at) VALUES ($1, $2, $3, $4)',
    [input.project_id, input.title, input.start_at, input.end_at]
  );
  return result.lastInsertId as number;
}

export async function updateEvent(id: number, input: UpdateEventInput): Promise<void> {
