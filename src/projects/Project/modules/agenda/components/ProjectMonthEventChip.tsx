...
import { fromLocalISO, minutesSinceMidnight, formatMinutesLabel, formatDuration } from '@/lib/utils/date';
import type { Event } from '@/types/event.types';

interface ProjectMonthEventChipProps {
  event: Event;
  color: string;
...
export function ProjectMonthEventChip({ event, color, onEdit, onDoubleClick }: ProjectMonthEventChipProps) {
  ...
  return (
    ...
    >
      {isHovering && (
        <div style={{ position: 'absolute', top: -20, left: 0, backgroundColor: '#333', color: '#fff', fontSize: 10, padding: '2px 5px', borderRadius: 4, whiteSpace: 'nowrap', zIndex: 10, pointerEvents: 'none' }}>
